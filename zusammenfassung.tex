...
V = a_{p}*p + a_{1}*v_{1} + a_{2}*v_{2} + ...		// v_{i} = Eigenvektor

\subsubsection{Freier Wille}
F: Surger setzt freien Wille ein

P(S_{j}') = P(S_{j}'|F)*P(F) + P(S_{j}'|\overline{F})*P(\overline{F})

P(F) = 1-\alpha
P(\overline{F}) = \alpha
P(S_{j}'|\overline{F})*P(\overline{F}) = \alpha Hp
P(S_{j}'|F)*P(F) = \frac{1}{N}*(1-\alpha)

G = Google-Matrix
A = Matrix voller Einsen (!= Einheitsmatrix)
p =  Gp = \alpha HP + (1-\alpha)*\frac{1}{N}*A*P --> \fbox{ G = \alpha H + (1-\alpha)*\frac{1}{N}*A }
